...
func newEnforcementCriteria(opts *Options) (verification.EnforcementCriteria, error) {
	...
	if opts.SANRegex != "" || opts.SAN != "" {
		...
		validatedWorkflowRegex, err := validateSignerWorkflow(opts.SignerWorkflow, opts.Hostname)
		if err != nil {
...
func validateSignerWorkflow(signerWorkflow, hostname string) (string, error) {
